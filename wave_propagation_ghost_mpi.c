...
static int gx(int lx){return g_d.mx+(g_dc->group_tiles[0].x_begin+lx-HALO);}
static int gy(int ly){return g_d.my+(g_dc->group_tiles[0].y_begin+ly-HALO);}
...
static void halo_x(Field*f){
  int nx=g_dc->group_tiles[0].nx,ny=g_dc->group_tiles[0].ny,s=f->stride,tag=100;
...
static void comp_interior(Field*f){
  ...
  if(g_d.nb_u>=0)ye=ny;
  for(int y=yb;y<ye;y++){int gy_=gy(y);if(gy_==0||gy_==NY-1)continue;
    for(int x=xb;x<xe;x++){int gx_=gx(x);if(gx_==0||gx_==NX-1)continue;
      double u=f->u_curr[F(f,y,x)];
      double d2x=(f->u_curr[F(f,y,x-1)]-2*u+f->u_curr[F(f,y,x+1)])/(DX*DX);
      double d2y=(f->u_curr[F(f,y-1,x)]-2*u+f->u_curr[F(f,y+1,x)])/(DY*DY);
      f->u_next[F(f,y,x)]=2*u-f->u_prev[F(f,y,x)]+C0*C0*DT2*(d2x+d2y);}}
}

static void comp_boundary(Field*f){
  const mythread_tile*t=&g_dc->group_tiles[0];int ny=t->ny,nx=t->nx;
  int lr=HALO,ur=ny,lc=HALO,rc=nx;
  if(g_d.nb_d>=0)for(int x=HALO;x<HALO+nx;x++){
    int gx_=gx(x);if(gx_==0||gx_==NX-1)continue;
    double u=f->u_curr[F(f,lr,x)];
    double d2x=(f->u_curr[F(f,lr,x-1)]-2*u+f->u_curr[F(f,lr,x+1)])/(DX*DX);
    double d2y=(f->u_curr[F(f,lr-1,x)]-2*u+f->u_curr[F(f,lr+1,x)])/(DY*DY);
    f->u_next[F(f,lr,x)]=2*u-f->u_prev[F(f,lr,x)]+C0*C0*DT2*(d2x+d2y);}
  if(g_d.nb_u>=0)for(int x=HALO;x<HALO+nx;x++){
    int gx_=gx(x);if(gx_==0||gx_==NX-1)continue;
    double u=f->u_curr[F(f,ur,x)];
    double d2x=(f->u_curr[F(f,ur,x-1)]-2*u+f->u_curr[F(f,ur,x+1)])/(DX*DX);
    double d2y=(f->u_curr[F(f,ur-1,x)]-2*u+f->u_curr[F(f,ur+1,x)])/(DY*DY);
    f->u_next[F(f,ur,x)]=2*u-f->u_prev[F(f,ur,x)]+C0*C0*DT2*(d2x+d2y);}
}
...
int main(int argc,char**argv){
  ...
  if(mr==0)printf("=== Wave MPI %dx%dx%d procs=%d (%dx%d) DT=%.4f CFL=%.4f ===\n",
    NX,NY,NT,ms,g_d.px,g_d.py,cfg_DT,cfg_C0*cfg_DT/cfg_DX);
