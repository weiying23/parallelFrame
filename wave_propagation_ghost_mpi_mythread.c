...
static int    cfg_HALO=1,cfg_N_WORKERS=2;
...
int main(int argc,char**argv){
  ...
  if(mr==0)printf("=== Wave MPI+mythread-sync %dx%dx%d procs=%d (%dx%d) workers/rank=%d DT=%.4f CFL=%.4f ===\n",
    NX,NY,NT,ms,g_d.px,g_d.py,cfg_N_WORKERS,cfg_DT,cfg_C0*cfg_DT/cfg_DX);
