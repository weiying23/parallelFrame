...
#define COURANT (C0 * DT / DX)
#define COURANT2 (COURANT * COURANT)
...
static void seed_initial_condition(void) {
  double cx = 0.5 * (NX - 1) * DX;
  double cy = 0.5 * (NY - 1) * DX;
  double sigma = 0.06 * NY * DX;
...
static void exchange_y_halos(void) {
  ...
  enforce_dirichlet_boundaries(g_sim.u_curr);

  if (g_sim.neighbor_up >= 0) {
    memcpy(g_send_up, &g_sim.u_curr[idx(g_sim.local_ny, 0)], (size_t)NX * sizeof(double));
    MPI_Sendrecv(
      g_send_up, NX, MPI_DOUBLE, g_sim.neighbor_up, 100,
      g_recv_up, NX, MPI_DOUBLE, g_sim.neighbor_up, 101,
      MPI_COMM_WORLD, &status
    );
    memcpy(&g_sim.u_curr[idx(g_sim.local_ny + HALO, 0)], g_recv_up, (size_t)NX * sizeof(double));
  } else {
    memset(&g_sim.u_curr[idx(g_sim.local_ny + HALO, 0)], 0, (size_t)NX * sizeof(double));
  }

  if (g_sim.neighbor_down >= 0) {
    memcpy(g_send_down, &g_sim.u_curr[idx(HALO, 0)], (size_t)NX * sizeof(double));
    MPI_Sendrecv(
      g_send_down, NX, MPI_DOUBLE, g_sim.neighbor_down, 101,
      g_recv_down, NX, MPI_DOUBLE, g_sim.neighbor_down, 100,
      MPI_COMM_WORLD, &status
    );
    memcpy(&g_sim.u_curr[idx(0, 0)], g_recv_down, (size_t)NX * sizeof(double));
  } else {
    memset(&g_sim.u_curr[idx(0, 0)], 0, (size_t)NX * sizeof(double));
  }

  enforce_dirichlet_boundaries(g_sim.u_curr);
}

static double compute_block(ThreadTask *task) {
  double energy = 0.0;

  for (int y = task->y_begin; y < task->y_end; y++) {
    for (int x = task->x_begin; x < task->x_end; x++) {
      double u_ij = g_sim.u_curr[idx(y, x)];
      double lap = g_sim.u_curr[idx(y - 1, x)] + g_sim.u_curr[idx(y + 1, x)]
                 + g_sim.u_curr[idx(y, x - 1)] + g_sim.u_curr[idx(y, x + 1)]
                 - 4.0 * u_ij;
      double next = 2.0 * u_ij - g_sim.u_prev[idx(y, x)] + COURANT2 * lap;
  ...
  task->energy = energy * DX * DX;
  return task->energy;
}

static double compute_current_energy_local(void) {
  double energy = 0.0;

  for (int y = HALO; y < g_sim.local_ny + HALO; y++) {
    for (int x = 1; x < NX - 1; x++) {
      double value = g_sim.u_curr[idx(y, x)];
      energy += value * value;
    }
  }
...
static void main_thread(void) {
  ...
  g_sim.initial_energy = compute_current_energy_local();
  MPI_Allreduce(&g_sim.initial_energy, &global_energy, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
