...
#define ENERGY_REPORT_INTERVAL 300
#endif
...
static void task_compute_energy(void *ctx) {
  ...
  do {
    old.d = *acc;
    new.d = old.d + partial;
  } while (!__sync_bool_compare_and_swap((volatile uint64_t*)acc, old.i, new.i));
}
...
static void setup_group_thread_tasks(void) {
  ...
  for (int g = 0; g < md.ngrp; g++) {
    ...
    for (int i = 0; i < n_chunks; i++) {
      ...
      plan->tasks[i].energy_acc = &g_group_energy[g];
    }
...
static void setup_single_group_tasks(void) {
  ...
  for (int i = 0; i < g_n_flat_tasks; i++) {
    ...
    g_flat_tasks[i].energy_acc = &g_energy_acc;
  }
